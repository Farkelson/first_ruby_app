...
class GameHelper
  ...
  def initialize(fname = '../data/SINGLE.TXT')
    ...
    @all_words.delete_if { |term| term.match(/\W/) }
  end
end
...
# This Array class will allow you to chain and find the specific word you're
# looking for.
class Array
  # filter the words to only those with size = len
  def with_word_length(len, len2 = 0)
    temp = select { |term| term.length == len }
    temp1 = select { |term| term.length == len2 }
    temp + temp1
  end
  ...
  def begins_with(*list_of_possible_letters)
    select { |term| list_of_possible_letters.any? { |letter| letter == term[0] } }
  end

  # filter the words to only those that start with
  # at least one of the passed-in letters
  def does_not_end_with(char)
    select { |term| term[-1] != char }
  end

  def char_count_less_than(char2, number)
    select { |term| term.count(char2) < number }
  end

  # filter the words to only those that contain
  # at least one of the passed-in letters
  def contains(*list_of_possible_letters)
    select { |term| list_of_possible_letters.any? { |letter| term.match(/#{letter}/) } }
  end
  ...
  def does_not_contain(*list_of_possible_letters)
    select { |term| list_of_possible_letters.none? { |letter| term.match(/#{letter}/) } }
  end
