...
# This class tests the game_helper class
class GameHelperTest < Minitest::Test
  ...
  def test_begins_with
    ...
    selected_terms.each { |term| assert_operator letters, :include?, term[0] }
  end
  ...
  def test_contains
    ...
    selected_terms.each { |term| assert term.match(/[#{letters.join}]/), "#{term} does not contain #{letters.join(',')}" }
  end
  ...
  def test_does_not_contain
    ...
    selected_terms.each { |term| assert term.match(/[^#{letters.join}]/), "#{term} contains #{letters.join(',')}" }
  end
